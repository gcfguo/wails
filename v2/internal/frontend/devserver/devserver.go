...
func (d *DevWebServer) Run(ctx context.Context) error {
	...
	assetdirValue := ctx.Value("assetdir")
	assetServer, err := assetserver.NewDevAssetServer(assetHandler, bindingsJSON, assetdirValue != nil, myLogger, runtime.RuntimeAssetsBundle)
	if err != nil {
		log.Fatal(err)
	}
